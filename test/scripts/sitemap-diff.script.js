import { isValidFilename, writeJsonToFile } from '../src/utils/fileUtils.ts';
...
try {
  ...
  const pathsDiff = await pathsDiff(sitemap1, sitemap2);
  ...
  if (outputFile) {
    if(writeJsonToFile(outputFile, pathsDiff)) {
      console.log(`Sitemap paths and differences successfully written to ${outputFile}`);
    }
  } else {
    if( pathsDiff.sitemap1PathsNotInSitemap2.length === 0 && pathsDiff.sitemap2PathsNotInSitemap1.length === 0) {
      console.log(`\nNo path differences found between the two sitemaps "${sitemap1}" and "${sitemap2}".`);
    } else {
      console.log(`\nDifferent paths found":`);
      if( pathsDiff.sitemap1PathsNotInSitemap2.length === 0) {
        console.log(`\nsitemap1 "${pathsDiff.sitemap1}" paths are all in sitemap2 "${pathsDiff.sitemap2}" paths.`);
      } else {
        console.log(`\nsitemap1 "${pathsDiff.sitemap1}" paths not in sitemap2\n${pathsDiff.sitemap2}\n`, pathsDiff.sitemap1PathsNotInSitemap2);
      }
      if( pathsDiff.sitemap2PathsNotInSitemap1.length === 0) {
        console.log(`\nsitemap2 "${pathsDiff.sitemap2}" paths are all in sitemap1 "${pathsDiff.sitemap1}" paths.`);
      } else {
        console.log(`\nSitemap2 "${pathsDiff.sitemap2}" paths not in sitemap1\n${pathsDiff.sitemap2}\n`, pathsDiff.sitemap1PathsNotInSitemap2);
      }
    }
    if(pathsDiff.commonPaths.length > 0) {
      console.log(`\nCommon paths found in both sitemaps:\n`, pathsDiff.commonPaths);
    } else {
      console.log(`\nNo common paths found between the two sitemaps "${pathsDiff.sitemap1}" and "${pathsDiff.sitemap2}".`);
    }
  }
} catch (error) {
